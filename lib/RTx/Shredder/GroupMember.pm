...
sub __DependsOn
{
	...
	$args{'Shredder'}->PutResolver(
			...
			Code => sub {
				my %args = (@_);
				my $group = $args{'TargetObj'};
				return if $args{'Shredder'}->GetState( Object => $group ) & FOR_WIPING;
				return unless ($group->Type || '') eq 'Owner';
				return unless ($group->Domain || '') eq 'RT::Ticket-Role';

				return if $group->MembersObj->Count > 1;

				my $group_member = $args{'BaseObj'};

				if( $group_member->MemberObj->id == $RT::Nobody->id ) {
					RTx::Shredder::Exception->throw( "Couldn't delete Nobody from owners role group" );
				}

				my( $status, $msg ) = $group->AddMember( $RT::Nobody->id );
				RTx::Shredder::Exception->throw( $msg ) unless $status;

				my $ticket = RT::Ticket->new( $group->CurrentUser );
				$ticket->Load( $group->Instance );
				RTx::Shredder::Exception->throw( "Couldn't load ticket" ) unless $ticket->id;
